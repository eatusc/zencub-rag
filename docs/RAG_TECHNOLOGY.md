...
This target is implemented as a first pass, but vector coverage is partial. TEST currently has 4,352 embedded chunks out of 12,104 total chunks. The broad text-search path remains the reliability baseline until the full embedding backfill is done.
...
The backfill process reads `rag_transcript_chunks.text`, sends each chunk to the embedding model, then writes the returned vector into `rag_transcript_chunks.embedding` with `embedded_at` and `embedding_model`. TEST currently has 4,352 embedded chunks and 7,752 chunks still missing vectors.
...
- `4,352` embedded chunks for semantic-search validation

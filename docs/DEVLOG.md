...
## 2026-08-06 - Five-instructor panel on the public compare app

Raised the public panel from three instructors to five after measuring whether the corpus supports it, rather than assuming it does not. Ran the same three topics at both sizes on gpt-4o-mini through the demo route, whose runs carry no surface marker and therefore stay out of the public listing. At five, all three panels filled completely, so no run emitted the "only N sufficiently supported instructors were available" caveat; claim verification scored 100% on all three against 67% for a three-instructor run of the same armbar question; evidence rose from 4-6 clips to 6-8; and the panels named more widely recognized instructors (Gordon Ryan, Bernardo Faria, Stephan Kesting, Nick Albin) where three had been returning obscure channels.

Verification quality improving with a wider panel is not a coincidence: `validate` requires every retained claim to cite at least two distinct instructors, and a five-person panel makes that bar easier to clear. Latency did not move (20-36s against 20-34s) because the analysis branches are a `Send` fan-out, so two more instructors are two more concurrent calls rather than two more sequential ones. Cost per run went from roughly $0.006 to $0.008, which moves the 500/day ceiling from about $3/day to $4/day.

Panel size is now `RAG_INSTRUCTORS_PANEL_SIZE`, clamped to the 2-5 the graph accepts, so it can be tuned from `serve.sh` without a code change. Live verification after deploy: "passing the de la riva guard" returned five instructors across seven clips in 27 seconds, 13 model calls, three of four claims verified, the fourth rejected by the verifier as an unsupported difference and dropped by `validate`. Shipped as `e9c0373`; 69 tests, typecheck, and lint pass.

## 2026-08-06 - Public Instructor Compare app at instructors.zencub.com

Added a third deployment surface (`APP_MODE=instructors`, port 3420, `.next-instructors`, launchd job `local.zencub-rag-instructors`) that runs the existing checkpointed comparison workflow as a one-question, one-button public app on gpt-4o-mini pinned server-side. The panel review node still executes and still records its decision in the trace, but auto-approves, because a public app cannot leave half-finished threads waiting on a click.

The workflow cannot answer inside a single request. Measured runs were 114 seconds on Qwen3 235B via OpenRouter and about 30 on gpt-4o-mini, while Cloudflare cuts an origin response off at 100 seconds and returns 524. So `POST /api/instructors/compare` starts a background job and returns a thread id, and the browser polls `GET` roughly once a second. `src/lib/instructorCompareJobs.ts` holds the live view in process, capped at four concurrent runs and pruned after thirty minutes; the durable copies remain the LangGraph checkpoint and the stored run row, so a restart mid-run costs a spinner rather than a result.

That poll turned out to be the most useful part of the interface. `runInstructorComparisonStreamed` uses `streamMode: "values"`, and because the trace channel has an append-only reducer, every chunk carries every node that has finished. The page therefore draws the real execution: three retrieval strategies fanning out and fusing, one analysis branch opening per instructor, those branches converging into a single synthesis, then one verifier branch per claim. Node ids come from the run, so nothing about the display is scripted.

Also shipped: same-thread follow-ups that reuse the approved panel out of the checkpoint (turn two of a local test reused six clips and kept the same three instructors), `/c/<uuid>` permalinks for stored runs, a recent-comparisons strip, and starter questions. Finished runs store with `result->>surface = 'instructors'`, and both the listing and the permalink filter on it, so nothing typed into the internal demo can appear on the public site. Follow-ups require the capability token minted with the thread; a thread id alone returns 403, verified against both a missing and a forged token.

Middleware answers 404 for every `/api` path outside `/api/instructors/compare`, `/api/instructors/runs`, and `/api/health`, and redirects any page path other than `/` and `/c/<uuid>` home. Verified through Cloudflare after deploy: `/api/rag/ask` and `/api/rag/instructor-compare` both 404 on this host. Shipped as `c0af859`.

One operational mistake worth recording. Creating the hostname with `cloudflared tunnel route dns` directly used the active `cert.pem`, which owns helpaproduct.com, and silently created `instructors.zencub.com.helpaproduct.com` rather than failing. This is exactly the trap `~/.cloudflared/README.md` documents, and the wrapper at `~/.cloudflared/route-dns` exists to prevent it. Recreated the record correctly with the wrapper, deleted the stray one through the Cloudflare API, and confirmed no other double-`.com` records remain in that zone. `docs/DEPLOYMENT.md` now tells the next person to use the wrapper and says why.

## 2026-08-06 - Production readiness review of the LangGraph surface

Reviewed the LangGraph side of the app for public exposure and fixed what the review found. Five items were real.

`LANGGRAPH_TEST_MODE` was live as `on` in production on demo.zencub.com. `.env.local` sets it for local testing, `serve.sh` overrode it only in the `public` branch, and the demo inherited it. Confirmed behaviorally rather than by inference: with a valid unlock cookie, `/api/rag/graph-note` and `/api/rag/graph-follow-up/checkpoints` both cleared the gate and returned validation errors instead of 403, meaning any PIN holder could inject failures into live graphs, replay checkpoints, and write rows into `rag_research_notes`. `serve.sh` now pins it off for every deployed surface; after deploy the same request returns "disabled outside explicit test mode".

The evidence gate spent its entire refinement budget on every run. Both measured runs ended at 2 of 2 rounds with the gaps unchanged, because the usual gap is that the corpus holds a single video of an instructor on the topic, which no query rewrite can fix, and each round paid for another rerank (13s and 31s of a 114s run). `canRefineComparison` now takes `lastRoundClosedAGap`, and `assess_panel` compares each round's gap count against the previous one, so a round has to close a gap to earn the next. Runs dropped to one round. The flag is shared with the final quality gate, which means a claim-verification repair is also skipped once targeted retrieval has proven unproductive; that is deliberate, since Loop B repairs by re-entering the same targeted retrieval and re-running every analysis, synthesis, and verification.

Model calls had no timeout anywhere except the Claude CLI and the Qwen probe, so a hung provider would hold the request, its checkpoint, and a socket indefinitely. Consolidated eleven direct `new OpenAI(...)` constructions into one `openaiFor(env)` factory with an explicit timeout (90s hosted, 240s local Qwen). This one also covered the public search host, since `/api/rag/ask` shares the same provider module.

Workflow routes returned raw exception text to the browser, which for the database paths meant Supabase messages and table names. Added `src/lib/workflowErrors.ts`: messages the graph raises deliberately pass through, everything else is logged server-side and reported generically. Workflow routes are also now rate limited at five per ten minutes per address, including the demo's, where the PIN had been the only thing bounding model spend, plus a site-wide comparison budget of 500/day.

Separately, the synthesis step's decision guide was coming back empty on most gpt-4o-mini runs. The prompt asks for it, but the model returns objects where `strings()` accepted only strings, so the parser silently emptied the one section a training reader acts on. Added a tolerant `textLines()` that flattens objects to their first meaningful text field.

## 2026-07-17 — One-shot LLM versus LangGraph consensus visual

Added a bottom System Map card contrasting a normal one-shot comparison prompt with the actual Instructor Compare fan-out/fan-in workflow. The visual explicitly identifies “independent analysis” as one separate call to the currently selected model per instructor—Qwen3 235B by default—with each call receiving only that instructor's clips. It then shows the later selected-model synthesis call, one verifier call per proposed claim, and the deterministic two-instructor citation gate that retains or removes the claim. The copy distinguishes orchestration reliability from model intelligence: LangGraph controls isolation, joins, verification, persistence, and traceability; it does not inherently make Qwen smarter.

## 2026-07-17 — Stored comparison run modal

Clarified that durable Instructor Compare history stores one record per completed initial run, follow-up turn, recovery result, or model-experiment branch, including local integration runs; failed attempts are not stored. The TEST database currently contains nine completed records: seven initial/legacy validation runs, one same-thread follow-up, and one GPT-4o Mini experiment branch. Added explicit run-type badges and made each table row a mouse- and keyboard-accessible trigger for a centered visual details modal. The modal contains the complete comparison, citations, model calls, tokens, verification results, quality gates, and LangGraph trace; it supports Escape/backdrop close and prevents background scrolling. No database change was required.

## 2026-07-17 — Instructor Compare adaptive, interruptible research workflow

Expanded Instructor Compare from a mostly forward pipeline into a durable LangGraph state machine. A deterministic evidence gate can route a weak panel through one bounded, model-authored targeted-retrieval round; guided runs then pause with a real `interrupt()` so the user can approve, remove clips, or reject before instructor analysis. Instructor `Send` branches still converge into synthesis, after which every proposed consensus/difference claim fans out to an independent selected-model verifier and must also pass deterministic two-instructor citation rules. The app now includes a visual state-machine map whose filled nodes come from the real trace and explicitly explains fan-out, joins, loop-back, pause/resume, follow-up state, recovery, and checkpoint branching.

Added capability-scoped same-thread follow-ups that reuse the approved panel, per-turn token/trace accounting, explicit failure recovery, and provider experiments cloned from the approved-panel checkpoint into a separate UUID thread with original/branch provenance. Added server-only `rag_instructor_compare_branch_cache` so successful instructor outputs are idempotently reused when a parallel superstep is retried. The new workflow migration also changes stored history uniqueness from one result per thread to one result per thread/turn. RLS remains enabled; `anon` and `authenticated` have no table privileges. Only `rag_*` tables and the existing isolated `langgraph` schema were used in the ZenCub TEST database.

Live Qwen3 235B TEST evidence: thread `da76153b-74f6-4f24-92b1-21cd885f0e0d` detected a weak panel, ran one targeted loop, paused at 11 checkpoints, resumed three instructor branches, ran five verifier branches, scored 100%, and stored turn one. Turn two reused four clips on the same thread; strict verification removed two unsupported overstatements and disclosed a 67% score. A GPT-4o Mini experiment cloned approved checkpoint `1f1824af-0775-6821-801d-5c19df334d80` into branch `812d6d3d-7cd3-48f8-a851-a8112b7789d3`, ran only downstream analysis/synthesis/verification, and left the original thread unchanged. Deterministic recovery thread `618a7cd8-d939-4aa4-b77e-661450398181` returned a recoverable 500, then completed with execution counts Jonathan Thomas 2 and John Danaher 1 plus one failure marker, proving only the failed model branch re-executed. The final compatibility integration passed with 19 checkpoints, three instructors, five cited videos, four retained claims, no private state/capability leakage, and valid request rejection. `npm run typecheck`, production `npm run build`, `npm audit --omit=dev` (zero vulnerabilities), and `git diff --check` passed.

## 2026-07-17 — Stored Instructor Compare quality history

Added durable Instructor Compare result storage and a bottom-of-tab quality explorer. Successful comparisons are inserted into server-only `rag_instructor_compare_runs` with provider/model, timing, prompt/output/total tokens, evidence and instructor counts, consensus/difference counts, and the complete already-redacted API result. The UI loads up to 100 newest runs, filters by text or provider, summarizes quality signals, and reopens the full instructor analyses, citations, consensus, differences, caveats, model calls, and LangGraph trace after browser/server restarts. Private vector/keyword/metadata candidate pools and checkpoint values are never stored in this table.

Applied `docs/migrations/2026-07-17-instructor-compare-history.sql` only to the configured ZenCub TEST database. RLS is enabled, browser grants are revoked, `service_role` receives only SELECT/INSERT, and a direct anon-key verification returned HTTP 401. The first live persisted Qwen3 235B run stored thread `8ee4f251-bfbd-4869-bd17-d7f7c2e18a5b`: 5,198 generation tokens, four evidence videos, three supported consensus claims, two differences, and no private-state keys in the stored/API representation.
